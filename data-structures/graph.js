...
class Node {
    ...
    connect(val) {
        this.edgesList.push(val);
        val.edgesList.push(this);
    }
    isConnected(val) {
        if (this.edgesList.includes(val)) return true;
        return false;
    }
    getAdjacency() {
        this.edgesList.forEach(item => console.log(item));
    }
}
...
class Graph {
    ...
    breadthFirstTraversal(start, finish) {
        let visitedNodes = new Set();
        let queue = [start];
        visitedNodes.add(start);
        while (queue.length > 0) {
            let current = queue.shift();
            if (current.val  === finish.val) {
                console.log('Found it!')
                return;
            } else {
                for (let adjacency of current.edgesList) {
    ...
    depthFirstTraversal(start, finish, visitedNodes = new Set()) {
        if (start.val === finish.val) {
            console.log('Found it!')
            return;
        }
        visitedNodes.add(start);
        console.log('Looking at Node', start.val);
        for (let adjacency of start.edgesList) {
            if (!visitedNodes.has(adjacency)) {
                this.depthFirstTraversal(adjacency, finish, visitedNodes);
            }
        }
    }
}
